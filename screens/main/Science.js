...
const Science = ({ useFirebase = false }) => {
    ...
    useEffect(() => {
        if (useFirebase) {
            ...
            fetchArticlesByCategory('science'); // Fetch bài viết thuộc thể loại kinh doanh
        }
    }, [useFirebase]);
    ...
    const fetchArticlesFromFirebase = async () => {
        try {
            ...
            const snapshot = await articlesRef.where('category', '==', category).get();
